...
//  var file = 'file' in params ? params.file : DEFAULT_URL;
  var file = $('#pdfviewer').attr('data-src');
  console.log('Initializing PDF %s', file);
...
$(function(){
    ...
    function initPDF(PDFPath, canvas) {
...
//                canvas.width = viewport.width;
    ...
    $('.pdfviewer').each(function(){
        var pdf_container = $(this);
        var pdf_url = pdf_container.attr('data-src');
        var canvas = $('<canvas>').appendTo(pdf_container);

        initPDF(pdf_url, canvas.get(0))
    });
})
